<a href="{{ route('keranjang') }}" class="relative p-2 text-gray-500 hover:bg-gray-100 rounded-full transition">
    ...
    @if($totalItem > 0)
        <span class="absolute top-1 right-0.5 min-w-[18px] h-[18px] bg-red-500 text-white text-[10px] font-bold flex items-center justify-center rounded-full border-2 border-white animate-bounce">{{ $totalItem }}</span>
    @endif
</a>

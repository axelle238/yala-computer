<div wire:poll.10s class="space-y-10">
    ...
    <div class="flex flex-col lg:flex-row gap-6 items-center justify-between bg-[#1E293B]/80 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/5 shadow-2xl">
        <div class="flex items-center gap-6 w-full lg:w-auto">
            <div class="w-16 h-16 bg-emerald-600/10 rounded-[1.5rem] flex items-center justify-center text-emerald-400 shadow-[inset_0_0_20px_rgba(16,185,129,0.1)]">
                <i class="fas fa-satellite-dish text-2xl animate-pulse"></i>
            </div>
            <div>
                <h2 class="text-2xl font-black text-white tracking-tighter italic uppercase">TRANSACTION TERMINAL</h2>
        ...
        <div class="flex flex-wrap items-center gap-4 w-full lg:w-auto">
            <div class="relative flex-1 lg:w-96 group">
                <input wire:model.live.debounce.300ms="cari" type="text" placeholder="Scanning invoice packets..." class="w-full bg-[#0F172A] border border-white/10 rounded-2xl py-4 px-6 pl-14 text-sm text-white focus:ring-2 focus:ring-emerald-500/50 outline-none transition-all group-hover:border-white/20">
                <i class="fas fa-search-dollar absolute left-6 top-4.5 text-slate-500 group-hover:text-emerald-500 transition-colors"></i>
            </div>
            <select wire:model.live="status_filter" class="bg-[#0F172A] border border-white/10 rounded-2xl py-4 px-6 text-sm text-slate-400 focus:ring-2 focus:ring-emerald-500/50 outline-none cursor-pointer hover:bg-white/5 transition-all font-black uppercase tracking-widest">
    ...
    <div class="flex flex-col xl:flex-row gap-10 items-start">
        ...
        <!-- Live Data Stream -->
        <div class="flex-1 w-full bg-[#1E293B]/60 backdrop-blur-xl rounded-[3rem] border border-white/5 overflow-hidden shadow-2xl">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-white/2 border-b border-white/5">
                            <th class="px-10 py-8 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Transaction ID</th>
                            <th class="px-10 py-8 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Source Entity</th>
                            <th class="px-10 py-8 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Valuation</th>
                            <th class="px-10 py-8 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Workflow Phase</th>
                            <th class="px-10 py-8 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] text-right">Ops</th>
                        </tr>
                    </thead>
                            ...
                            <tr class="hover:bg-emerald-600/5 transition-all group cursor-pointer {{ $pesanan_dipilih && $pesanan_dipilih->id == $pesanan->id ? 'bg-emerald-600/10 border-l-4 border-emerald-500' : '' }}" wire:click="lihatDetail({{ $pesanan->id }})">
                                <td class="px-10 py-8">
                                    <div class="flex flex-col gap-2">
                                        ...
                                        <p class="text-[9px] text-slate-500 font-mono italic uppercase">{{ $pesanan->created_at->format('Y.m.d // H:i') }}</p>
                                    </div>
                                </td>
                                <td class="px-10 py-8">
                                    <div class="flex items-center gap-4">
                                        <div class="w-12 h-12 rounded-2xl bg-slate-700/50 border border-white/10 flex items-center justify-center text-white font-black text-xs shadow-lg group-hover:scale-110 transition-transform">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-[10px] text-slate-500 font-mono tracking-tighter">{{ $pesanan->pelanggan->surel ?? '-' }}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-10 py-8">
                                    ...
                                    @php
                                        $warnaBayar = [
                                            ...
                                            'gagal' => 'text-red-400'
                                        ][$pesanan->status_pembayaran] ?? 'text-slate-500';
                                    @endphp
                                ...
                                <td class="px-10 py-8">
                                    @php
                                        $warnaStatus = [
                                            'baru' => 'border-blue-500/30 text-blue-400 bg-blue-500/5',
                                            'diproses' => 'border-orange-500/30 text-orange-400 bg-orange-500/5',
                                            'dikirim' => 'border-purple-500/30 text-purple-400 bg-purple-500/5',
                                            'selesai' => 'border-emerald-500/30 text-emerald-400 bg-emerald-500/5',
                                            'batal' => 'border-red-500/30 text-red-400 bg-red-500/5'
                                        ][$pesanan->status_pesanan] ?? 'border-slate-500 text-slate-500';
                                    @endphp
                                ...
                                <td class="px-10 py-8 text-right">
                                    <div class="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-slate-500 group-hover:text-emerald-400 transition-all">
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-10 py-40 text-center">
                                    ...
                                    <p class="text-slate-500 font-mono italic tracking-widest">No transaction packets detected in current orbit.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
        ...
        @if($pesanan_dipilih)
            <div class="w-full xl:w-[500px] flex-shrink-0 animate-fade-in-right">
                <div class="bg-[#1E293B]/90 backdrop-blur-3xl rounded-[3rem] border border-emerald-500/30 shadow-2xl shadow-emerald-900/20 sticky top-28 overflow-hidden">
                    
                    <!-- HUD Header -->
                    <div class="p-10 border-b border-white/5 bg-gradient-to-r from-emerald-600/10 to-transparent flex items-center justify-between">
                        <div>
                        ...
                        <button wire:click="tutupDetail" class="w-12 h-12 rounded-2xl hover:bg-white/5 text-slate-500 hover:text-white transition-all border border-white/10">
                    ...
                    <div class="p-10 space-y-10 max-h-[70vh] overflow-y-auto custom-scrollbar">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div class="bg-[#0F172A]/50 p-6 rounded-3xl border border-white/5">
                                <label class="block text-[9px] font-black text-slate-500 uppercase tracking-widest mb-4">Payment Matrix</label>
                                <select wire:change="updateStatus('pembayaran', $event.target.value)" class="w-full bg-transparent border-none text-emerald-400 font-black text-xs p-0 focus:ring-0 cursor-pointer">
                                    <option value="pending" class="bg-[#1E293B]" {{ $pesanan_dipilih->status_pembayaran == 'pending' ? 'selected' : '' }}>PENDING</option>
                                    <option value="lunas" class="bg-[#1E293B]" {{ $pesanan_dipilih->status_pembayaran == 'lunas' ? 'selected' : '' }}>SYNCHRONIZED</option>
                                    <option value="gagal" class="bg-[#1E293B]" {{ $pesanan_dipilih->status_pembayaran == 'gagal' ? 'selected' : '' }}>ABORTED</option>
                                </select>
                            </div>
                            <div class="bg-[#0F172A]/50 p-6 rounded-3xl border border-white/5">
                                <label class="block text-[9px] font-black text-slate-500 uppercase tracking-widest mb-4">Workflow Control</label>
                                <select wire:change="updateStatus('pesanan', $event.target.value)" class="w-full bg-transparent border-none text-blue-400 font-black text-xs p-0 focus:ring-0 cursor-pointer">
                                    @foreach(['baru', 'diproses', 'dikirim', 'selesai', 'batal'] as $st)
                                        <option value="{{ $st }}" class="bg-[#1E293B]" {{ $pesanan_dipilih->status_pesanan == $st ? 'selected' : '' }}>{{ strtoupper($st) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="bg-white/2 p-8 rounded-[2rem] border border-white/5 relative overflow-hidden">
                            ...
                            <h4 class="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-6 flex items-center gap-3">
                            ...
                            <div class="space-y-4 font-mono text-xs">
                                ...
                                <div class="flex flex-col gap-2">
                                    <span class="text-slate-500">GEO_LOCATION:</span>
                            ...
                            <div class="space-y-4">
                                @foreach($pesanan_dipilih->item as $item)
                                    <div class="flex gap-5 items-center p-4 bg-white/2 rounded-2xl border border-white/5 hover:bg-white/5 transition-all">
                                        <div class="w-12 h-12 rounded-xl bg-[#0F172A] border border-white/10 flex items-center justify-center text-emerald-500 overflow-hidden">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <div class="flex justify-between mt-2 font-mono text-[9px]">
                                                <span class="text-slate-500">QNT: {{ $item->kuantitas }}</span>
                        ...
                        <div class="pt-10 border-t border-white/10">
                            <div class="flex justify-between items-center bg-emerald-500/10 p-8 rounded-3xl border border-emerald-500/20 shadow-[inset_0_0_30px_rgba(16,185,129,0.05)]">
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-[9px] text-slate-500 font-mono mt-1 italic">Taxes Included</span>
                                </div>

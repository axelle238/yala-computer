<div wire:poll.15s class="space-y-10">
    ...
    <div class="flex flex-col lg:flex-row gap-6 items-center justify-between bg-[#0B1120]/50 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/5 shadow-2xl">
        ...
        <div class="flex items-center gap-6 w-full lg:w-auto">
            <div class="relative flex-1 lg:w-96 group">
                <input wire:model.live.debounce.300ms="cari" type="text" placeholder="Scanning entity biometrics (Name, Email, Tel)..." class="w-full bg-white/2 border border-white/10 rounded-2xl py-4 px-6 pl-14 text-sm text-white focus:ring-2 focus:ring-orange-500/50 outline-none transition-all group-hover:border-white/20 font-mono italic">
                <i class="fas fa-fingerprint absolute left-6 top-4.5 text-gray-500 group-hover:text-orange-500 transition-colors"></i>
            </div>
            <div class="flex flex-col items-end">
                <span class="text-[10px] font-black text-gray-500 tracking-widest uppercase mb-1">Active Nodes</span>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        @forelse($daftar_pelanggan as $pel)
            <div class="bg-[#0B1120]/50 backdrop-blur-xl rounded-[2.5rem] border border-white/5 p-8 shadow-xl hover:border-orange-500/30 transition-all group relative overflow-hidden">
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex items-center gap-5 mb-8">
                        <div class="relative">
                            ...
                            <div class="absolute -bottom-1 -right-1 w-5 h-5 bg-emerald-500 rounded-full border-4 border-[#0B1120] shadow-[0_0_10px_#10b981]"></div>
                        </div>
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-[9px] font-mono text-gray-500 uppercase tracking-tighter truncate">{{ $pel->surel }}</p>
                        </div>
                    </div>

                    <!-- Technical Specs -->
                    <div class="space-y-4 mb-8 bg-black/40 p-6 rounded-2xl border border-white/5">
                        <div class="flex items-center gap-4 text-[10px] font-mono">
                            <i class="fas fa-phone text-orange-500 w-4"></i>
                            <span class="text-gray-400">COM_PORT:</span>
                            <span class="text-white font-bold">{{ $pel->telepon ?? 'NOT_SET' }}</span>
                        </div>
                        <div class="flex items-start gap-4 text-[10px] font-mono">
                            ...
                            <span class="text-white font-bold italic line-clamp-2 leading-relaxed">{{ $pel->alamat ?? 'GEO_DATA_NULL' }}</span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-6 border-t border-white/5">
                        <div class="flex flex-col">
                            <span class="text-[9px] font-black text-gray-600 uppercase tracking-[0.2em] mb-1">Loyalty Index</span>
                        ...
                        <button class="bg-white/5 hover:bg-orange-600 text-gray-400 hover:text-white px-5 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest transition-all border border-white/5 hover:border-orange-500 shadow-lg">
                            Fetch Logs
                        </button>
                    </div>
                </div>
        ...
        @empty
            <div class="col-span-full bg-[#0B1120]/50 backdrop-blur-xl rounded-[3rem] border border-white/5 p-32 text-center">
                <i class="fas fa-user-slash text-white/5 text-8xl mb-8"></i>
                <p class="text-gray-600 font-mono italic tracking-[0.3em] uppercase">No human entities detected in current database scan.</p>
            </div>
        @endforelse
    </div>

    <!-- Pagination Terminal -->
    <div class="mt-12 bg-[#0B1120]/30 p-6 rounded-[2rem] border border-white/5 shadow-inner">

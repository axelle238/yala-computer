<div class="space-y-10">
    @if(!$apakah_menambah && !$apakah_mengedit)
        ...
        <div class="flex flex-col lg:flex-row gap-6 items-center justify-between bg-[#1E293B]/80 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/5 shadow-2xl">
            ...
            <div class="flex flex-wrap items-center gap-4 w-full lg:w-auto">
                <div class="relative flex-1 lg:w-96 group">
                    <input wire:model.live="cari" type="text" placeholder="Scanning content headlines..." class="w-full bg-[#0F172A] border border-white/10 rounded-2xl py-4 px-6 pl-14 text-sm text-slate-300 focus:ring-2 focus:ring-blue-400/50 outline-none transition-all placeholder-slate-600">
                    <i class="fas fa-radar absolute left-6 top-4.5 text-slate-500 group-hover:text-blue-400"></i>
                </div>
        ...
        <div class="bg-[#1E293B]/60 backdrop-blur-xl rounded-[3.5rem] border border-white/5 overflow-hidden shadow-2xl">
            <div class="overflow-x-auto">
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-white/2 border-b border-white/5">
                            <th class="px-10 py-8 text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Headline Node</th>
                            <th class="px-10 py-8 text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Channel</th>
                            <th class="px-10 py-8 text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Auth User</th>
                            <th class="px-10 py-8 text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Broadcast Status</th>
                            <th class="px-10 py-8 text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] text-center">Ops</th>
                        </tr>
                    </thead>
                        ...
                        @forelse($daftar_artikel as $art)
                            <tr class="hover:bg-blue-400/5 transition-all group">
                                <td class="px-10 py-8">
                                    ...
                                    <p class="text-[9px] font-mono text-slate-500 mt-2 tracking-tighter">ENDPOINT: /{{ $art->slug }}</p>
                                </td>
                                <td class="px-10 py-8">
                                    <span class="px-4 py-1.5 rounded-lg bg-white/5 border border-white/10 text-[9px] font-black text-slate-400 uppercase tracking-widest">{{ $art->kategori }}</span>
                                </td>
                                <td class="px-10 py-8">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="text-xs font-bold text-slate-500">{{ $art->penulis->nama ?? 'SYSTEM_CORE' }}</span>
                                    </div>
                                </td>
                                <td class="px-10 py-8">
                                    @php
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-10 py-40 text-center">
                                    ...
                                    <p class="text-slate-500 font-mono italic tracking-[0.2em]">Listening for transmission... No active signals detected.</p>
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
    ...
    @else
        ...
        <div class="max-w-[1400px] mx-auto animate-fade-in-up">
            <div class="bg-[#1E293B]/90 backdrop-blur-3xl rounded-[4rem] border border-blue-400/20 shadow-2xl overflow-hidden">
                <!-- Editor Header -->
                <div class="p-12 border-b border-white/5 bg-gradient-to-r from-blue-600/20 via-transparent to-transparent flex items-center justify-between">
                    ...
                    <button wire:click="batal" class="w-16 h-16 rounded-[1.5rem] bg-white/2 border border-white/10 text-slate-500 hover:text-red-500 hover:border-red-500/50 transition-all flex items-center justify-center text-2xl">
                    ...
                    <div class="grid grid-cols-1 xl:grid-cols-4 gap-16">
                        ...
                        <div class="xl:col-span-3 space-y-10">
                            <div class="relative group">
                                <label class="block text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-4 ml-2">Headline Identification</label>
                                <input wire:model="judul" type="text" class="w-full bg-[#0F172A] border border-white/10 rounded-[2rem] py-6 px-10 text-2xl font-black text-white focus:ring-4 focus:ring-blue-500/20 outline-none transition-all placeholder-slate-700 italic uppercase tracking-tight" placeholder="PROTOCOL TITLE...">
                            ...
                            <div class="relative group">
                                <label class="block text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-4 ml-2">Content Matrix Data</label>
                                <textarea wire:model="konten" rows="18" class="w-full bg-[#0F172A] border border-white/10 rounded-[2.5rem] py-8 px-10 text-sm text-slate-400 focus:ring-4 focus:ring-blue-500/20 outline-none transition-all leading-relaxed font-mono" placeholder="// Initiate content stream here..."></textarea>
                        ...
                        <div class="space-y-10">
                            <!-- Config Hub -->
                            <div class="bg-[#0F172A]/50 p-8 rounded-[2.5rem] border border-white/5 space-y-8">
                                <h3 class="text-[10px] font-black text-blue-400 uppercase tracking-[0.3em] mb-6">Config Hub</h3>
                                
                                <div class="space-y-6">
                                    <div>
                                        <label class="block text-[9px] font-black text-slate-600 mb-3 uppercase tracking-widest">Frequency Channel</label>
                                        <select wire:model="kategori" class="w-full bg-[#1E293B] border border-white/10 rounded-xl py-4 px-6 text-xs text-white font-bold outline-none focus:ring-2 focus:ring-blue-500/50 transition-all cursor-pointer">
                                            <option value="Berita" class="bg-[#0F172A]">BROADCAST</option>
                                            <option value="Tutorial" class="bg-[#0F172A]">HANDBOOK</option>
                                            <option value="Review" class="bg-[#0F172A]">ANALYSIS</option>
                                            <option value="Tips" class="bg-[#0F172A]">INSIGHTS</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="pt-4">
                                        <label class="flex items-center justify-between p-4 bg-white/2 rounded-2xl border border-white/5 cursor-pointer group hover:bg-white/5 transition-all">
                                            <span class="text-[10px] font-black text-slate-500 group-hover:text-emerald-400 uppercase tracking-widest">Live Sync</span>
                                            <div class="relative">
                                                <input type="checkbox" wire:model="apakah_diterbitkan" class="sr-only peer">
                                                <div class="w-14 h-7 bg-slate-800 rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-1 after:left-[4px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-emerald-500 shadow-inner"></div>
                                            </div>
                                        </label>
                                    </div>
                                </div>
                            ...
                            <div class="pt-10 space-y-4">
                                ...
                                <button type="button" wire:click="batal" class="w-full bg-white/2 hover:bg-white/5 text-slate-500 font-black py-5 rounded-[2rem] text-[10px] uppercase tracking-[0.3em] transition-all border border-white/5">
                                    Abort Process
                                </button>
                            </div>
                        </div>
